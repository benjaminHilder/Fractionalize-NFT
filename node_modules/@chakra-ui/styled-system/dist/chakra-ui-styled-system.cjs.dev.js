...
var withoutImportant = function withoutImportant(value) {
  return utils.isString(value) ? value.replace(/!(important)?$/, "").trim() : value;
};
...
var tokenToCSSVar = function tokenToCSSVar(scale, value) {
  return function (theme) {
    ...
    var transformed = utils.isObject(theme.__cssMap) && key in theme.__cssMap ? theme.__cssMap[key].varRef : value;
...
function normalize(value, toArray) {
  ...
  if (utils.isObject(value)) return toArray(value);
  if (value != null) return [value];
...
function createResolver(theme) {
  ...
  return function resolver(config, prop, value, props) {
    ...
    var _loop = function _loop(i) {
      ...
      var query = utils.toMediaQueryString(key.minW, nextKey == null ? void 0 : nextKey._minW);
      var styles = utils.runIfFn((_config$prop = config[prop]) == null ? void 0 : _config$prop[normalized[i]], props);
      ...
      if (isMultipart) {
        ...
        (_config$parts = config.parts) == null ? void 0 : _config$parts.forEach(function (part) {
          ...
          utils.mergeWith(result, (_mergeWith = {}, _mergeWith[part] = isSingle ? styles[part] : (_ref = {}, _ref[query] = styles[part], _ref), _mergeWith));
        });
        return "continue";
      }

      if (!isMultipart) {
        if (isSingle) utils.mergeWith(result, styles);else result[query] = styles;
        return "continue";
      }
...
function resolveStyleConfig(config) {
  return function (props) {
    ...
    return utils.mergeWith({}, utils.runIfFn((_config$baseStyle = config.baseStyle) != null ? _config$baseStyle : {}, props), recipe(config, "sizes", size, props), recipe(config, "variants", variant, props));
  };
}
